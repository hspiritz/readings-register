...
st.markdown("""
    <style>
        ...
        div[data-baseweb="select"] > div, div[data-baseweb="input"] > div, div[data-baseweb="textarea"] > div, input, textarea {
            background-color: #120a06 !important; color: #ffffff !important; border: 1px solid #6b4c3a !important;
        }
        label, p, span, .stWidgetLabel { color: #ffffff !important; font-weight: 600 !important; }
        .mobile-sidebar-hint {
            background-color: #120a06; border: 2px dashed #00F0FF; padding: 10px; border-radius: 6px; margin-bottom: 15px; font-size: 14px; text-align: right; color: #ffffff;
        }
        .vn-blue, em, h2 span, h3 span, p span { color: #00F0FF !important; font-weight: normal !important; font-style: normal !important; }
        button[kind="primaryFormSubmit"], .stFormSubmitButton > button {
            background-color: #120a06 !important; color: #ffffff !important; border: 1px solid #8c644d !important; font-weight: bold !important; transition: all 0.2s ease;
        }
        button[kind="primaryFormSubmit"]:hover, .stFormSubmitButton > button:hover { background-color: #2c1d16 !important; border-color: #ffffff !important; }
        h1, h2, h3, h4 { margin-top: 2px !important; margin-bottom: 4px !important; padding-top: 0px !important; padding-bottom: 0px !important; color: #ffffff !important; font-weight: bold !important; }
        div.block-container { padding-top: 0.5rem !important; padding-bottom: 1rem !important; }
        .blue-divider { border: 0; height: 1px; background: #00F0FF; margin-top: 15px; margin-bottom: 15px; }
    </style>
...
MAP_METRIC = {
    "Chapters": "Chapters / Số Chương", "Sessions": "Sessions / Số Lần Đọc",
    "Days Read": "Days Read / Số Ngày Đọc", "Missed Days": "Missed Days / Số Ngày Chưa Đọc",
    "Reading Rate": "Reading Rate / % Số Ngày Đọc"
}

USER_DIRECTORY = {
    "Katelyn": "+60193763635", "Thoa": "+601155080892", "Ruth": "+60183668919",
    "Sarah": "+60168412926", "Tham": "+60169159217", "Tina": "+601121103011",
    "Hannah": "+60182499896", "hs": "+60122193637"
}

st.markdown('<div class="mobile-sidebar-hint">📱 <b>Mobile Users:</b> Tap the double arrow <b>( >> )</b> button in the top-left corner to open the Entry Form!<br><span class="vn-blue">Thành viên dùng điện thoại: Nhấn dấu mũi tên <b>( >> )</b> ở góc trên cùng bên trái để mở Đơn Nhập Liệu!</span></div>', unsafe_allow_html=True)
    ...
    if selected_name == "Others" and (not custom_name or not custom_phone):
        ...
        if target_worksheet is not None:
            try:
                ...
                st.cache_data.clear()
                st.rerun()
            except Exception as write_err: st.sidebar.error(f"Write error: {write_err}")
        else: st.sidebar.error("Database connection was unavailable.")
...
if has_data:
    ...
    st.markdown(f"<div style='margin-top: 5px; margin-bottom: 5px;'><span style='font-size: 16px; font-weight: bold; color: #ffffff;'>📅 {T_DATE_HDR}</span><span class='vn-blue' style='font-size: 13px;'>{T_DATE_VIE} : </span><span style='font-size: 24px; font-weight: bold; color: #00F0FF; margin-left: 5px;'>{today.strftime('%d-%m-%Y')}</span></div>", unsafe_allow_html=True)
    hdr_col, opt_col = st.columns([2, 3])
    with hdr_col: st.markdown(f"<div style='padding-top:6px;'><span style='font-size: 16px; font-weight: bold; color: #ffffff;'>⏱️ {T_PERIOD_HDR}</span> <span class='vn-blue' style='font-size: 13px;'>{T_PERIOD_VIE}</span></div>", unsafe_allow_html=True)
    with opt_col: filter_mode = st.radio("Period Tracking Options Mode", options=[f"{T_CURR_MONTH} / {T_CURR_MONTH_VIE}", f"{T_CUST_RANGE} / {T_CUST_RANGE_VIE}"], horizontal=True, label_visibility="collapsed")
    ...
    if f"{T_CUST_RANGE} / {T_CUST_RANGE_VIE}" in filter_mode:
        ...
        start_filter, end_filter = today.replace(day=1), today
    ...
    if len(filtered_df) > 0:
        ...
        for user in all_users:
            ...
            calculated_records.append({
                "Name": user, "Chapters": ch_count, "Sessions": sess_count, "Days Read": days_read_count, "Missed Days": missed_days_count, "Reading Rate": 0, "PctText": f"{pct_val}%"
            })
        ...
        for idx, row in calc_df.iterrows():
            ...
            fig.add_annotation(x=total_bar_length, y=row["Name"], text=row["PctText"], showarrow=False, xshift=22, font=dict(color="#00F0FF", size=24, weight="bold"))
        
        fig.update_layout(
            margin=dict(l=10, r=60, t=10, b=10), paper_bgcolor='#1e120b', plot_bgcolor='#1e120b', font=dict(color="#ffffff", size=13),
            xaxis=dict(title=dict(text="Count / Số Lượng", font=dict(color="#ffffff", size=14)), tickfont=dict(color="#ffffff", size=12), gridcolor="#422d22", showline=True, linewidth=1, linecolor='#00F0FF'),
            yaxis=dict(title=dict(text="Users / Người Đọc", font=dict(color="#ffffff", size=14)), tickfont=dict(color="#ffffff", size=13), showline=True, linewidth=1, linecolor='#00F0FF'),
            legend=dict(orientation="h", y=-0.28, x=0.5, xanchor="center", title=dict(text="Metrics / Tiêu Chí:", font=dict(color="#ffffff", size=12)), font=dict(color="#ffffff", size=12))
        )
        ...
        if not user_stats.empty:
            ...
            <table style="width:100%; border-collapse: collapse; background-color: #2c1d16; border: 1px solid #422d22; font-size:13px; margin-bottom:10px;">
                ...
                <tr>
                    <td style="padding: 8px; border: 1px solid #422d22; color: #f5f0eb; vertical-align: top;">Hover over graph menu bar. Click the 📷 <b>Camera Icon</b> to instantly download chart image file.</td>
                    <td style="padding: 8px; border: 1px solid #422d22; color: #f5f0eb; vertical-align: top;">Press green link block below. Click the <b>Paperclip (+) attachment icon</b> in WhatsApp, choose saved file, then hit Send.</td>
                </tr>
            </table>
            """, unsafe_allow_html=True)
            if phone_num: st.markdown(f'<a href="{whatsapp_url}" target="_blank"><button style="padding:10px; background-color:#7c5c43; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold; font-size:14px; width:100%;">{T_WA_LAUNCH}</button></a>', unsafe_allow_html=True)
